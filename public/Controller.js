...
Controller.prototype.start = function () {
  ...
  socket.on("setup", function (data) {
    // https://stackoverflow.com/a/35195944/28145
    ...
    if (password == null) password = lastPassword;
    ...
    for (let id of ids) {
      ...
      if (structure.parent != null) {
        let botUI = new BotUi(
          id,
          structure.structure,
          self.botUIs[structure.parent],
          structure.attachTarget
        );
        botUI.create();
  ...
  socket.on("updateBotUI", function (data) {
    for (var i in data) {
      if (self.botUIs[i]) self.botUIs[i].update(data[i]);
    }
  });
  ...
  socket.on("createBotUI", function (dataBotUI) {
    if (dataBotUI.parent != null) {
      let botUI = new BotUi(
        dataBotUI.id,
        dataBotUI.structure,
        self.botUIs[dataBotUI.parent],
        dataBotUI.attachTarget
      );
      botUI.create();
